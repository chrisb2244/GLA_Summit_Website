...
const submitFn = jest.fn<void, [Readonly<FormData>, BaseSyntheticEvent | undefined]>(async (data, event) => {})
...
describe('Person', () => {
  ...
  it('rejects an invalid email without @ symbol', async () => {
    ...
    expect(errors[0]).toHaveTextContent('This email doesn\'t match the expected pattern')
  })
  ...
  it('accepts an email with the @ symbol and suitable pattern', async () => {
    ...
    await screen.findAllByRole('alert').then((foundElems) => {
