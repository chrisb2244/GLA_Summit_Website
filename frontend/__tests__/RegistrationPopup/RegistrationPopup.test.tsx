...
import { SignInOptions, SignInReturn, SignUpOptions, SignUpReturn, UserCredentials } from '@/lib/sessionContext'
...
const goodSignUpValue = async (creds: UserCredentials, opts?: SignUpOptions): Promise<SignUpReturn> => {
  return {
    user: dummyUser,
    session: dummySession,
    error: null
...
describe('RegistrationPopup', () => {
  const signInFn = jest.fn<Promise<SignInReturn>, [string, SignInOptions | undefined]>()
  const signUpFn = jest.fn<Promise<SignUpReturn>, [UserCredentials, SignUpOptions | undefined]>()
