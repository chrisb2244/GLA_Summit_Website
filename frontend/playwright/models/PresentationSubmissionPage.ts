...
type PresentationFormData = Omit<SubmissionData, 'submitter' | 'otherPresenters' | 'timeWindows'>
...
export class PresentationSubmissionPage {
  ...
  constructor(page: Page) {
    ...
    this.learningPointsInput = this.page.locator('label:has-text("Learning Points")')
    this.presentationTypeInput = this.page.locator('label:has-text("Presentation Type") >> xpath=.. >> role=button')
  ...
  async fillFormData(data: Partial<PresentationFormData>) {
    if (typeof data.title !== 'undefined') await this.titleInput.fill(data.title)
    if (typeof data.abstract !== 'undefined') await this.abstractInput.fill(data.abstract)
    if (typeof data.learningPoints !== 'undefined') await this.learningPointsInput.fill(data.learningPoints)
    ...
    if (typeof data.isFinal !== 'undefined') {
      await (data.isFinal ? this.isFinalInput.check() : this.isFinalInput.uncheck())
    }
  }
}
