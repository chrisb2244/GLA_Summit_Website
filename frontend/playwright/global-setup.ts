...
async function globalSetup(config: FullConfig) {
  ...
  await page.locator('role=dialog', { hasText: /Thank you.*check.*email/i }).waitFor({state: 'visible'});

  expect(requests).toHaveLength(2);
  const userId = requests[0]['userId']
  const emailText = requests[1]['text'] as string
  const activationLink = emailText.match(/Please use this link to confirm your email address: (.*)The non-HTML version/)?.[1]
